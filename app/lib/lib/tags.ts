...
export function inferBucket(title?: string, summary?: string): Bucket | null {
  ...
  if (/(mobili[aá]rio|furniture|cadeira|sof[aá])/.test(hay)) return 'mobiliario';
  if (/(interiores|interior design|casa|apartamento)/.test(hay)) return 'interiores';
  if (/(sustent|regener|circular|reuso|reutiliza)/.test(hay)) return 'sustentabilidade';
  if (/(arquitet|architecture|urban)/.test(hay)) return 'arquitetura';
  if (/(\\bai\\b|intelig[êe]ncia artificial|generativa)/.test(hay)) return 'ia';
